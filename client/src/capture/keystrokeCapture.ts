...
export class KeystrokeCapture {
  ...
  snapshot(): KeyStats { const mean = (xs: number[]) => xs.length ? xs.reduce((a, b) => a + b, 0) / xs.length : 120; const avg = mean(this.dwell); const variance = this.dwell.length > 1 ? this.dwell.reduce((s, x) => s + (x - avg) ** 2, 0) / this.dwell.length : 40; return { typing_speed: Math.min(1, this.dwell.length / 75), dwell_mean: avg / 200, flight_mean: mean(this.flight) / 300, digraph_mean: mean(this.digraph) / 300, typing_variance: Math.min(1, variance / 5000), timing_entropy: Math.min(1, (new Set(this.dwell.map(x => Math.round(x / 10))).size) / 30) }; }
}
